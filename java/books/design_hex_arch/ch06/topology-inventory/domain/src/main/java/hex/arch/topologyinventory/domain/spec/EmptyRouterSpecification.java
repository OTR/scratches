...
public final class EmptyRouterSpecification
    extends AbstractSpecification<CoreRouter> {
    ...
    public void check(CoreRouter coreRouter) {
        if (!isSatisfiedBy(coreRouter)) {
            throw new GenericSpecificationException(
                "It isn't allowed to remove a core router" +
                " with other routers attached to it"
            );
        }
    }
