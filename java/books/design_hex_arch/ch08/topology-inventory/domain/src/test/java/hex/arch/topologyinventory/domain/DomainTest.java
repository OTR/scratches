...
import hex.arch.topologyinventory.domain.service.RouterService;
import hex.arch.topologyinventory.domain.service.SwitchService;
...
public class DomainTest {
    ...
    public void addSwitchToEdgeRouter() {
        Location location = createLocation("US");
        Switch networkSwitch = createSwitch("30.0.0.0", 8, location);
        EdgeRouter edgeRouter = createEdgeRouter(location, "30.0.0.0");
    ...
    public void addEdgeToCoreRouter(){
        ...
        CoreRouter coreRouter = createCoreRouter(location, "40.0.0.1");
    ...
    public void addEdgeToCoreRouter_failBecauseRoutersOfDifferentCountries(){
        Location locationUS = createLocation("US");
        Location locationJP = createLocation("JP");
        EdgeRouter edgeRouter = createEdgeRouter(locationUS,"30.0.0.1");
        CoreRouter coreRouter = createCoreRouter(locationJP, "40.0.0.1");
    ...
    public void filterRouterByVendor(){
        ...
        var actualVendor = RouterService
            .filterAndRetrieveRouter(routers,
            Router.getVendorPredicate(Vendor.HP))
            .get(0).getVendor();
        assertEquals(Vendor.HP, actualVendor);

        actualVendor = RouterService
            .filterAndRetrieveRouter(routers,
            Router.getVendorPredicate(Vendor.CISCO))
            .get(0).getVendor();
    ...
    public void filterSwitchByType(){
        ...
        var actualSwitchType = SwitchService
            .filterAndRetrieveSwitch(switches,
    ...
    public void filterNetworkByProtocol(){
        ...
        var actualProtocol = NetworkService
            .filterAndRetrieveNetworks(
                networks,
            Switch
                .getNetworkProtocolPredicate(Protocol.IPV4)
            )
            .get(0)
            .getNetworkAddress()
            .getProtocol();
    ...
    public void findRouterById(){
        ...
        Id expectedId = newCoreRouter.getId();
        Id actualId = RouterService.findById(
            routersOfCoreRouter, expectedId
        ).getId();
        assertEquals(expectedId, actualId);
    }
    ...
    private Switch createSwitch(String address, int cidr, Location location){
        Network newNetwork = createTestNetwork(address, cidr);
        List<Network> networks = createNetworks(newNetwork);
        Switch networkSwitch = createNetworkSwitch(location, networks);
        return networkSwitch;
    }

    private Switch createNetworkSwitch(
