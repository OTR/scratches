...
import static hex.arch.topologyinventory.domain.vo.RouterType.CORE;
import static hex.arch.topologyinventory.domain.vo.RouterType.EDGE;
...
public class RouterRemove extends ApplicationTestData {
    ...
    public void the_core_router_has_at_least_one_edge_router_connected_to_it() {
        var predicate = Router.getRouterTypePredicate(
            EDGE);
    ...
    public void i_remove_the_core_router_from_another_core_router() {
        ...
        var expectedId = this.coreRouter.removeRouter(
            coreRouterToBeRemoved).getId();
        assertEquals(expectedId, actualId);
    }
